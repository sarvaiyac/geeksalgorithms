...
//	final String serverNA_filename = "/Users/chintansarvaiya/Documents/Workspace_kepler/DBAssignment3/server-NA-data.txt";
	...
	public String createPlayerAccount(String firstname, String lastname,
		...
		try {
			if (MethodHelper.isValid(username, password)) { // check if username and password are valid
				boolean isRegistered = isUserRegistered(username); 

				if (isRegistered) { // check if registered then give error
					msg = "Username already registered";
				} else { // if not registered then register
					...
					FileWriter fwriter = new FileWriter(serverNA_filename, true);
	...
	public String playerSignIn(String username, String password,
		...
		boolean isRegistered = isUserRegistered(username);
		if (isRegistered) { // check if registered then make it sign in
			try {
				...
				br.close();
				FileOutputStream file = new FileOutputStream(serverNA_filename);
	...
	public String getPlayerStatus(String username,String password, String ipaddress) {
		...
		if (username.equalsIgnoreCase("admin") && password.equalsIgnoreCase("admin")) {
			...
			String result1 = "NA : " + getStatus() + "\n";
			String result2 = "EU : " + getReplyFromUDP("getplayerstatus", 1102)
					+ "\n";
			String result3 = "AS : " + getReplyFromUDP("getplayerstatus", 1103)
					+ "\n";
	...
	public String transferAccount(String username, String password,
		...
		if (isRegistered) { // if user is registered then transfer user to appropriate region using udp
			...
			if (_newgeolocation.equalsIgnoreCase(_geolocation)) {
				...
				if (msg.equalsIgnoreCase("User transferred")){
				...
				String _strToPass = getAllUserDetailsInOneStringToTransfer(username,newipaddress);
				msg = getReplyFromUDP(_strToPass, 1103);
				if (msg.equalsIgnoreCase("User transferred")){
					suspendAccount("admin", "admin", oldipaddress, username);
					msg = "account transferred to " + _newgeolocation + " ( "
							+ newipaddress + " ) with username : " + username;					
		...
		} else { // if user is not registered give error
	...
	public boolean isUserRegistered(String username) {
		try {
			FileInputStream fInStream = new FileInputStream(serverNA_filename);
			DataInputStream in = new DataInputStream(fInStream);
