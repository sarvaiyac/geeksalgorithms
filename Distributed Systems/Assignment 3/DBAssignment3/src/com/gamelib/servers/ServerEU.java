...
//	final String serverEU_filename = "/Users/chintansarvaiya/Documents/Workspace_kepler/DBAssignment3/server-EU-data.txt";
		...
		try {
			...
			if (MethodHelper.isValid(username, password)) { // check if given username and password are valid
				...
				if (isRegistered) { // check if registered
					...
					FileWriter fwriter = new FileWriter(serverEU_filename, true);
	...
	public String playerSignIn(String username, String password,
		...
		if (isRegistered) { // check if user is registered
			try {
				...
				br.close();
				FileOutputStream file = new FileOutputStream(serverEU_filename);
	...
	public boolean isUserRegistered(String username) {
		try {
			FileInputStream fInStream = new FileInputStream(serverEU_filename);
			DataInputStream in = new DataInputStream(fInStream);
